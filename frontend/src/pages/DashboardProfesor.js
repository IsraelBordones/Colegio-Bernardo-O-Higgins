...
function DashboardProfesor({ usuario, onLogout }) {
  ...
  const handleGuardarNota = async (e) => {
    ...
    try {
      const payload = {
        alumnoId: alumnoSeleccionado.username,
      ...
      setShowNota(false);
      setNuevaNota({ asignatura: '', tipo: 'Prueba', calificacion: '', periodo: 1 });
      const nuevasNotas = await listarNotas(alumnoSeleccionado.username);
  ...
  const handleRegistrarAsistencia = async (e) => {
    ...
    try {
      ...
      setShowAsistencia(false);
      const nueva = await historialAsistencia(alumnoSeleccionado.username);
      setAsistencia(nueva || []);
    } catch (e) {
          ...
          {!alumnoSeleccionado ? (
            ...
            <>
              <div className="detalle-header">
                <h2>{alumnoSeleccionado.nombre || alumnoSeleccionado.username}</h2>
                <span className="alumno-sub">@{alumnoSeleccionado.username}</span>
              </div>
